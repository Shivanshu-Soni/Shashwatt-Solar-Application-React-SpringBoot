...
function InstallationInvoice() {
  ...
  let customerId = installationDetails.customer.id;
  const handleDownload = () => {
      if (customerId) {
        axios({
          url: `http://localhost:8181/downloadFileByCustomerId/${customerId}`,
          method: 'GET',
    ...
    const  handleSendMail = async() =>{
        try {
            const response = await axios.get(`http://localhost:8181/downloadAndSendEmail/${customerId}`);
      
            if (response.status === 200) {
              setResultMessage(response.data);
            } else {
              setResultMessage('An error occurred while sending the email.');
            }
          } catch (error) {
...
export default InstallationInvoice;
